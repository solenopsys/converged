...
/** Fallback language used when no language is provided or a localized
 *  context is missing. Mirrors the storage convention `<lang>/<chatId>.json`
 *  (see data/converged/struct-ms — locale is the top-level key segment). */
...
export class ContextStoreService {
  ...
  /** Localized key: `<lang>/<chatId>.json`. */
  private buildKey(chatId: string, language: string): string {
    return `${language}/${chatId}.json`;
  }

  /** Pre-i18n flat key, kept only so existing data keeps resolving. */
  ...
  async getContext(
    ...
    // Resolution order: requested language → default language → legacy flat key.
    const candidates = [
  ...
  async listContexts(
    ...
    for (const key of keys) {
      ...
      try {
        const decoded = Buffer.from(stored).toString("utf-8");
        const parsed = JSON.parse(decoded) as Partial<ChatContext>;
        // key is either `<lang>/<id>.json` or legacy `<id>.json`.
