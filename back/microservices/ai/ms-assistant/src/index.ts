...
async function publishBusinessEvent(
	...
	try {
		...
		console.warn("[ms-assistant] Failed to publish business event", error);
	}
}
...
function resolveServiceType(value?: string): ServiceType | undefined {
	...
	if (normalized === "claude" || normalized === "anthropic")
		return ServiceType.ANTHROPIC;
	if (
		normalized === "gemini" ||
		normalized === "gemin" ||
		normalized === "google"
	)
		return ServiceType.GEMINI;
...
class ChatsServiceImpl {
	...
	constructor(config: {
		...
		this.defaultServiceType = resolveServiceType(
			process.env[CHAT_PROVIDER_ENV],
		);
	...
	async registerChat(threadId: string, title?: string): Promise<Chat> {
		...
		void publishBusinessEvent("chat.created", threadId);
	...
	async recordChatFile(threadId: string, fileSize?: number): Promise<Chat> {
		await this.init();
		const conversation = await this.stores.metadataService.recordFile(
			threadId,
			fileSize,
		);
