/**
 * llm-gate-bridge — background sync from llm-audio-gate → ms-calls
 *
 * After call sessions end in llm-audio-gate this bridge:
...
type GateTranscriptResp = { ok: boolean; session: string; transcript: GateTranscriptItem[] };
...
export class LlmGateBridge {
  ...
  start(): void {
    ...
    console.info(`[llm-gate-bridge] started — syncing every ${this.intervalMs}ms from ${this.gateUrl}`);
  }
  ...
  async sync(): Promise<void> {
    ...
    try {
      ...
      // Gate may be down — normal when running without llm-audio-gate
      console.debug("[llm-gate-bridge] gate unreachable:", (err as Error).message);
      return;
    }
    ...
    for (const sessionId of sessions) {
      ...
      try {
        ...
        console.warn(`[llm-gate-bridge] failed to sync session ${sessionId}:`, err);
      }
    }
  }
  ...
  private async fetchTranscript(sessionId: string): Promise<GateTranscriptItem[]> {
    try {
      const r = await fetch(
