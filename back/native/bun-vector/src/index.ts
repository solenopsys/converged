...
const DEFAULT_ENTRYPOINT = "sqlite3_vec_init";
const EXTENSION_ENV_VAR = "BUN_SQLITE_VEC_EXTENSION";
...
function resolveArch(runtimeArch: string): string {
  switch (runtimeArch) {
    ...
    default:
      throw new Error(
        `Unsupported architecture '${runtimeArch}' for sqlite-vec native bindings`,
      );
  }
}
...
function getLibPath(): string {
  ...
  const filename = `libsqlite-vec-${arch}-${libc}.so`;
...
export interface SqliteVecLoadOptions {
...
export interface SqliteVecDatabaseOptions
  extends DatabaseOptions, SqliteVecLoadOptions {}

export class SqliteVecDatabase extends Database {
  constructor(filename: string, options: SqliteVecDatabaseOptions = {}) {
    ...
    loadSqliteVec(this, { extensionPath, entryPoint });
  }
}
