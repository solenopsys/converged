...
export class RuntimeServiceImpl {
	...
	constructor(config?: RuntimeServiceConfig) {
		...
		const workflowList: any[] = Array.isArray(workflows.WORKFLOWS)
			? workflows.WORKFLOWS
			: [];
		...
		this.scriptResolver = new RuntimeScriptResolver({
			...
			checkIntervalMs: Number(
				process.env.RUNTIME_SCRIPT_HASH_CHECK_INTERVAL_MS || 0,
			),
			refreshIntervalMs: Number(
				process.env.RUNTIME_SCRIPT_HASH_REFRESH_INTERVAL_MS || 0,
			),
		...
		if (this.hasFeature("cron")) {
			this.cronEngine = new CronEngine((record) => {
				console.log(
					`[runtime] cron fired: ${record.cronName} success=${record.success}`,
				);
				void this.recordCronHistory(record);
			});

			const refreshIntervalMs = Number(
				process.env.CRON_REFRESH_INTERVAL_MS || 30000,
			);
		...
		if (typeof config?.registerShutdownTask === "function") {
			config.registerShutdownTask(
				`runtime:${Array.from(this.features).join("+")}:timers`,
				async () => {
	...
	private scheduleStartupSync(): void {
		const configuredDelay = Number(
			process.env.RUNTIME_STARTUP_SYNC_DELAY_MS ?? 1000,
		);
		const delayMs =
	...
	private async waitForServicesReady(): Promise<boolean> {
		const timeoutMs = Number(
			process.env.RUNTIME_SERVICES_READY_TIMEOUT_MS ?? 30000,
		);
		const intervalMs = Number(
			process.env.RUNTIME_SERVICES_READY_INTERVAL_MS ?? 1000,
		);
		const deadline =
			Date.now() +
		...
		console.info(
			`[runtime] startup sync skipped: services not ready at ${healthUrl}`,
		);
		return false;
	}
	...
	private isTemporaryHostUnavailable(error: unknown): boolean {
		...
		if (
			message.includes("storage error: storenotfound") ||
	...
	private logDagPersistenceFailure(action: string, error: unknown): void {
		if (this.isTemporaryHostUnavailable(error)) {
			...
			console.info(
				`[runtime] ${action} skipped: dag host temporarily unavailable (${details})`,
			);
			return;
		}
	...
	private async tryDagPersistence<T>(
		action: string,
		fn: () => Promise<T>,
	): Promise<T | undefined> {
		try {
			...
			this.logDagPersistenceFailure(action, error);
			return undefined;
		}
	}

	private logRefreshCronsFailure(
		stage: "initial" | "periodic",
		error: unknown,
	): void {
		if (this.isTemporaryHostUnavailable(error)) {
			const details =
				error instanceof Error
					...
					: String(error ?? "unknown error");
			console.info(
				`[runtime] refreshCrons ${stage}: sheduller host temporarily unavailable (${details})`,
			);
			return;
		}
		...
		if (this.isTemporaryHostUnavailable(error)) {
			const details =
				error instanceof Error
					...
					: String(error ?? "unknown error");
			console.info(
				`[runtime] resumeActiveExecutions skipped: dag host temporarily unavailable (${details})`,
			);
			return;
		}
			...
			if (this.isTemporaryHostUnavailable(error)) {
				const details =
					error instanceof Error
						...
						: String(error ?? "unknown error");
				console.info(
					`[runtime] recordCronHistory skipped: sheduller host temporarily unavailable (${details})`,
				);
				return;
			}
	...
	private async getCachedNodeResult(
		executionId: string,
		nodeId: string,
		...
		>("getCachedNodeResult");
		return (
			(await this.tryDagPersistence("getCachedNodeResult", () =>
				fn(executionId, nodeId),
			)) ?? { hit: false }
		);
	}

	private async loadVarsCache(): Promise<Map<string, any>> {
		const response = await this.tryDagPersistence("listVars", () =>
			this.dagClient.listVars(),
		);
	...
	private async runExecution(
		...
		try {
			if (!options.skipOpen) {
				await this.tryDagPersistence("openExecution", () =>
					openExecution(executionId, workflowName, params),
				);
			}
			...
			if (!Ctor) {
				await this.tryDagPersistence("setExecutionStatus", () =>
					setExecutionStatus(executionId, "failed"),
				);
				emit({
					type: "failed",
					executionId,
			...
			const ctx = {
				runNode: async (
					...
					const taskTicket = (await this.tryDagPersistence("createTask", () =>
						createTask(workflowId, nodeName),
					)) ?? { id: -Date.now(), createdAt: Date.now() };
					const createdAt = Number(taskTicket?.createdAt ?? Date.now());
					const persistedTaskId =
						Number.isFinite(taskTicket.id) && taskTicket.id > 0
							? taskTicket.id
							: undefined;
					emit({
						type: "task_update",
					...
					if (persistedTaskId !== undefined) {
						await this.tryDagPersistence("setTaskProcessing", () =>
							setTaskProcessing(persistedTaskId, startedAt),
						);
					}
					if (DAG_NODE_LOG) {
					...
					try {
						...
						if (persistedTaskId !== undefined) {
							await this.tryDagPersistence("setTaskDone", () =>
								setTaskDone(
									persistedTaskId,
									workflowId,
									nodeName,
						...
						if (persistedTaskId !== undefined) {
							await this.tryDagPersistence("setTaskFailed", () =>
								setTaskFailed(persistedTaskId, completedAt, errorMessage),
							);
						}
						emit({
							type: "task_update",
				...
				setStatus: (
					...
					void this.tryDagPersistence("setExecutionStatus", () =>
				...
				setVar: (key: string, value: any) => {
					vars.set(key, value);
					void this.tryDagPersistence("setVar", () =>
						this.dagClient.setVar(key, value),
					);
				},
			...
			try {
				...
				await this.tryDagPersistence("setExecutionStatus", () =>
					setExecutionStatus(executionId, "done"),
				);
	...
	async *startExecution(
		...
		void this.runExecution(executionId, workflowName, params, {
			...
			.catch((error) => {
				console.error(
					`[runtime] startExecution failed id=${executionId} workflow=${workflowName}`,
					error,
				);
			})
			.finally(() => {
	...
	async createExecution(
		...
		void this.runExecution(executionId, workflowName, params).catch((error) => {
			console.error(
				`[runtime] createExecution failed id=${executionId} workflow=${workflowName}`,
				error,
			);
		});
	...
	async resumeActiveExecutions(limit = 200): Promise<ResumeExecutionsResult> {
		...
		for (const item of items) {
			...
			resumed += 1;
			ids.push(item.id);
			void this.runExecution(item.id, item.workflowName, item.params, {
				skipOpen: true,
			}).catch((error) => {
				console.error(
					`[runtime] resume execution failed id=${item.id} workflow=${item.workflowName}`,
					error,
				);
			});
		}
	...
	async listWorkflows(): Promise<{ names: string[] }> {
		try {
			...
			return {
				names: Array.from(
					new Set([...dynamicNames, ...this.workflowNames]),
				).sort(),
			};
		} catch (error) {
			if (!this.isTemporaryHostUnavailable(error)) {
	...
	private async resolveWorkflowCtor(
		workflowName: string,
		...
		try {
			...
			if (this.isTemporaryHostUnavailable(error)) {
				console.error(
					`[runtime] resolve workflow from ms-scripts temporarily unavailable workflow=${workflowName}`,
					error,
				);
				return undefined;
			}
	...
	async refreshCrons(): Promise<void> {
		...
		const active = Array.isArray((list as any)?.items)
			? ((list as any).items as Array<any>)
			: [];
