...
const dagOpenExecutionMock = mock(
	async (
		_id: string,
...
const dagSetExecutionStatusMock = mock(
	async (_id: string, _status: string) => {},
);
const dagGetCachedNodeResultMock = mock(
	async (_executionId: string, _nodeId: string) => ({ hit: false }),
);
const dagCreateTaskMock = mock(async () => ({ id: 1, createdAt: 10 }));
const dagSetTaskProcessingMock = mock(
	async (_taskId: number, _startedAt: number) => {},
);
const dagSetTaskDoneMock = mock(
	async (
		_taskId: number,
		_executionId: string,
		_nodeId: string,
...
const dagSetTaskFailedMock = mock(
	async (_taskId: number, _completedAt: number, _errorMessage: string) => {},
);
...
describe("Runtime cron history bridge with mocked cron engine", () => {
	...
	test("should expose workflow names from runtime config", async () => {
		const { service, shutdown } = await createRuntimeService({
			workflows: {
				WORKFLOWS: [{ name: "wf.alpha" }, { name: "wf.beta" }],
			},
		});

		const result = await service.listWorkflows();
		expect(result.names).toContain("wf.alpha");
		expect(result.names).toContain("wf.beta");
	...
	test("should complete workflow when dag task persistence is temporarily unavailable", async () => {
		class MockWorkflow {
			readonly id: string;
		...
		dagSetTaskDoneMock.mockRejectedValueOnce(connectionError);
