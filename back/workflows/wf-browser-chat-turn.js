// One browser-controlled chat turn. UI tools are returned to the browser and
// their results are supplied in the next invocation; Centimanus owns history.
rt.workflow = function (params) {
  ...
  var context = rt.node("load-context", function () {
    ...
    return rt.call("contexts", "getContext", {
  ...
  for (var i = 0; i < blocks.length; i++) {
    var block = blocks[i] || {};
    if (block.type === "text") {
      history.push({ role: "user", content: String(block.data || "") });
