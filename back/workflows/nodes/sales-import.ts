...
export type SalesImportContact = {
	type?: string;
	value?: string;
	role?: string;
	description?: string;
...
export type NormalizedSalesImportItem = {
	lead: Lead;
	contacts: Contact[];
	tags: string[];
};
...
export class SalesImportLoadTextSourcesNode implements INode {
	constructor(public name: string) {}

	async execute(params: SalesImportInput): Promise<{ sourceText: string }> {
		const texts: string[] = [];
		const clients = createStorageClients(params.servicesBaseUrl);

		for (const fileId of params.fileIds ?? []) {
			const stored = await readFileFromStorage(fileId, clients);
		...
		return { sourceText: texts.join("\n\n").trim() };
	}
}

export class SalesImportBuildEmptyResultNode implements INode {
	constructor(public name: string) {}

	async execute(): Promise<SalesImportResult> {
		return {
			status: "empty",
			parsed: 0,
			leadsCreated: 0,
...
export class SalesImportExtractLeadsNode implements INode {
	constructor(public name: string) {}

	async execute(data: {
		sourceText: string;
		params: SalesImportInput;
	}): Promise<{ rawLeads: SalesImportLead[] }> {
		let rawLeads: SalesImportLead[] = [];
		if (data.params.useAgent ?? true) {
			try {
				rawLeads = await analyzeWithAgent(data.sourceText, data.params);
			} catch (_error) {
				rawLeads = [];
			}
		}
		if (rawLeads.length === 0) rawLeads = tryParseJsonLeads(data.sourceText);
		if (rawLeads.length === 0) rawLeads = parseDelimitedLeads(data.sourceText);
		if (rawLeads.length === 0) rawLeads = parseLooseLineLeads(data.sourceText);

		return { rawLeads };
	}
}

export class SalesImportNormalizeLeadsNode implements INode {
	constructor(public name: string) {}

	async execute(data: {
		rawLeads: SalesImportLead[];
		params: SalesImportInput;
	}): Promise<{ normalized: NormalizedSalesImportItem[] }> {
		return { normalized: normalizeLeads(data.rawLeads, data.params) };
	}
}

export class SalesImportBuildResultNode implements INode {
	constructor(public name: string) {}

	async execute(data: {
		normalized: NormalizedSalesImportItem[];
		dryRun?: boolean;
	}): Promise<SalesImportResult> {
		return {
			status: data.dryRun ? "dry-run" : "imported",
			parsed: data.normalized.length,
			leadsCreated: 0,
			leadsSkipped: 0,
			contactsCreated: 0,
			contactsSkipped: 0,
			errors: [],
			items: data.normalized.map(({ lead, contacts }) => ({
				leadId: lead.id,
...
export class SalesImportPersistNode implements INode {
	constructor(public name: string) {}
