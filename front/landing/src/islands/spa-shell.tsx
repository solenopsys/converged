...
function buildDefaultMfEnv(locale: string): Record<string, unknown> {
	return {
		"mf-docs": {
			docs: [{ name: "cnc", id: `${locale}/product/docs/index.json` }],
		},
		"mf-landing": {
			landingConfId: `${locale}/product/landing/cnc-landing.json`,
			title: "Converged CNC",
		},
...
function readImportMapMicrofrontends(): string[] {
	...
	try {
		...
		return Object.keys(parsed?.imports ?? {}).filter((k) =>
			k.startsWith("mf-"),
		);
	} catch {
		return [];
...
const DEFAULT_GROUP: GroupDef = GROUPS.find((g) => g.id === "content") ?? {
	id: "other",
	title: "Other",
...
async function fetchAllowedMicrofrontends(): Promise<string[] | null> {
	...
	const pending = (async () => {
		try {
			...
			const names = rows
				.map((entry: any) =>
					normalizeMfName(entry?.name ?? entry?.module ?? entry?.id),
				)
	...
	for (const name of names) {
		...
		try {
			...
			if (typeof runtime.getMenu === "function") {
				try {
					const dm = await runtime.getMenu();
					if (dm) menu = dm;
				} catch (e) {
					console.error(`[mf] getMenu ${name}`, e);
				}
			}
			if (menu) {
			...
			console.error(`[mf] Failed ${name}`, e);
		}
	}
}
...
function presentGuestLanding() {
	...
	const run = () => {
		...
		if (!centerSlot) {
			if (attempt < maxAttempts) setTimeout(run, 80);
			return;
		}
		try {
