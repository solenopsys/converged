...
function useSidebar(side: "left" | "right" = "left") {
  ...
  const sideState = side === "right" ? context.right : context.left
  const open = context.isMobile ? sideState.openMobile : sideState.open
  const state = context.isMobile ? (open ? "expanded" : "collapsed") : sideState.state
...
function SidebarProvider({
  ...
  const leftState = openLeft ? "expanded" : "collapsed"
...
function Sidebar({
  ...
  const mergedStyle = (canResize
    ? ({ "--sidebar-width": `${resizableWidth}px`, ...style } as React.CSSProperties)
    : width
    ...
    return (
      ...
      >
        ...
        <div
          ...
          className={cn(
            "fixed inset-y-0 z-40 h-svh transition-[width,left,right] duration-200 ease-linear",
  ...
  const handleResizeStart = (event: React.PointerEvent<HTMLDivElement>) => {
    if (!canResize || resizableWidth === null) return
    event.preventDefault()
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      const clamped = Math.max(minWidth, Math.min(maxWidth, Math.round(nextWidth)))
      setResizableWidth(clamped)
      ...
      >
        {canResize && (
          <div
            ...
            className={cn(
              "absolute inset-y-0 w-3 cursor-col-resize touch-none",
              "flex items-center justify-center",
              side === "right" ? "-left-1.5" : "-right-1.5"
            )}
            onPointerDown={handleResizeStart}
          >
            <GripVertical className="size-3 text-muted-foreground/60 opacity-0 transition-opacity group-hover:opacity-100" />
          </div>
        )}
        <div
          data-sidebar="sidebar"
...
export default function SidebarTrigger({
  ...
  const { toggleSidebar, open } = useSidebar(side)
  ...
  return (
    ...
    >
      {side === "right"
        ? (localOpen ? <PanelRightCloseIcon className="size-4" /> : <PanelRightIcon className="size-4" />)
        : (localOpen ? <PanelLeftCloseIcon className="size-4" /> : <PanelLeftIcon className="size-4" />)}
