...
const runActionFromUriFx = createEffect((actionId: string): boolean => {
	...
	// Deep-link should always reveal the current app menu panel.
	controlPanelOpened();
	tabActivated(MENU_TAB_ID);
	...
	try {
		...
		console.error(
			"[right-rail uri-sync] failed to run action",
			actionId,
			error,
		);
		return false;
...
sample({
	...
	fn: (location) =>
		normalizeTab(new URLSearchParams(location.search).get(SIDEBAR_TAB_PARAM)),
...
sample({
	...
	fn: (location) =>
		normalizePanel(
			new URLSearchParams(location.search).get(SIDEBAR_PANEL_PARAM),
		),
...
sample({
	...
	fn: (panel) => (panel === "chat" ? CHAT_TAB_ID : MENU_TAB_ID),
	target: tabActivated,
});
...
sample({
	...
	filter: (pendingActionId, action) =>
		Boolean(pendingActionId) && action.id === pendingActionId,
