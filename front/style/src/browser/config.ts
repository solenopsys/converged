...
export default function init(inlineConfig: RuntimeOptions = {}) {
  ...
  if (runtimeOptions.autoPrefix) {
    const postprocessors = (userConfigDefaults.postprocess = toArray(
      userConfigDefaults.postprocess,
    ));
    postprocessors.unshift(
      autoPrefixer(defaultDocument.createElement("div").style),
    );
  }
  ...
  const inject = (styleElement: HTMLStyleElement) =>
    runtimeOptions.inject
      ? runtimeOptions.inject(styleElement)
      : html().prepend(styleElement);
  const rootElement = () =>
    runtimeOptions.rootElement
      ? runtimeOptions.rootElement()
  ...
  async function updateStyle() {
    ...
    result.layers.reduce((previous: string | undefined, current) => {
      getStyleElement(current, previous).innerHTML =
        result.getLayer(current) ?? "";
      return current;
    }, undefined);
  ...
  async function extract(str: string) {
    ...
    // Extract tokens from string using regex (applyExtractors removed in newer versions)
    const extracted = str.match(/[\w-/:]+/g) || [];
  ...
  function observe() {
    if (observing) return;
    const target = runtimeOptions.observer?.target
      ? runtimeOptions.observer.target()
      : rootElement();
    if (!target) return;
  ...
  const unoCssRuntime =
    (defaultWindow.__unocss_runtime =
    defaultWindow.__unocss_runtime =
      {
        version: uno.version,
        uno,
        ...
        toggleObserver(set) {
          if (set === undefined) paused = !paused;
          else paused = !!set;
