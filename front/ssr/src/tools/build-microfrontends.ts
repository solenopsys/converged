...
function resolveMicrofrontendEntry(
	root: string,
	name: string,
): string | undefined {
...
for (const name of microfrontends) {
	...
	const result = await Bun.build({
		...
		external: [
			...externalPackages,
			...microfrontends.filter((microfrontend) => microfrontend !== name),
		],
	...
	console.log(
		`[landing] built ${name}.js (${(jsBytes.byteLength / 1024).toFixed(1)}kb → ${(compressed.byteLength / 1024).toFixed(1)}kb br)`,
	);
}
