...
```ts
type DirectiveFunction = <T extends unknown[]>(
  ref: Element,
  ...args: T
) => void;
...
function createDirective<T extends unknown[] = []>(
  name: string,
  fn: DirectiveFunction<T>,
...
const App = () => {
  return (
    <input
      ref={TooltipDirective.ref("This is a tooltip!")}
      value="Placeholder..."
    />
  );
};
...
type LazyFetcher<P = {}> = () => Promise<
  { default: JSX.Component<P> } | JSX.Component<P>
>;
...
const App = () => {
  const heading = "h2";
  return <Dynamic component={heading}>Some content</Dynamic>;
};
```
...
```ts
function DynamicLazy<P = {}>(props: {
  component: "path [module]:[component]";
...
function ErrorBoundary(props: {
  fallback:
    | JSX.Element
    | ((props: { error: Error; reset: Callback }) => JSX.Element);
  children: JSX.Element;
...
function For<T>(props: {
  ...
  children: (value: T, index: FunctionMaybe<number>) => JSX.Element;
}): ObservableReadonly<JSX.Element>;
function For<T>(props: {
  values?: FunctionMaybe<readonly T[]>;
  fallback?: JSX.ELement;
...
```ts
type Truthy<T = unknown> = Extract<
  T,
  number | bigint | string | true | object | symbol | Function
>;

function If<T>(props: {
  when: FunctionMaybe<T>;
  fallback?: JSX.Element;
  children: JSX.Element | ((value: () => Truthy<T>) => JSX.Element);
}): ObservableReadonly<JSX.Element>;
```
...
function Portal(props: {
  ...
  wrapper?: JSX.Element;
  children: JSX.Element;
}): (() => JSX.Element | null) & { metadata: { portal: HTMLDivElement } };
```
...
function useEventListener(
  ...
  handler: ObservableMaybe<(event: Event) => void>,
...
): ObservableReadonly<Resource<Response>>;
```
...
const TooltipDirectiveImmediate = createDirective(
  "tooltip",
  (ref, title: string) => {
    // Implementation...
  },
...
const SomeConditionalComponent = (
  when: FunctionMaybe<boolean>,
  value: string,
): JSX.Element => {
  return <If when={when}>{value}</If>;
};
```
