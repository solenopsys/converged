...
const createChatWidget = (_bus: any, params?: { contextName?: string }) => ({
	view: ChatView,
	placement: () => "sidebar:right",
	// ChatView reads `contextName` from its config and binds it into the chat
	// session via initializeChat. Persist it here so a remount/re-present never
	// re-initializes the session with an undefined context (which would drop the
	// company/system prompt and make the assistant ask "which company?").
	config: { contextName: params?.contextName },
	commands: {},
});
...
const createShowChatAction: CreateAction<any> = (bus) => ({
	...
	invoke: (params?: { contextName?: string }) => {
		const widget = createChatWidget(bus, params);
