...
import type { PaginationParams, ScriptFile, ScriptListItem } from "g-scripts";
...
const listScriptsFx = domain.createEffect<PaginationParams, any>({
	name: "LIST_SCRIPTS",
	handler: async (params) => scriptsService.listScripts(params),
});

const readScriptFx = domain.createEffect<string, ScriptFile>({
	name: "READ_SCRIPT",
	handler: async (path) => scriptsService.readScript(path),
});

const saveScriptFx = domain.createEffect<ScriptFile, string>({
	name: "SAVE_SCRIPT",
	handler: async (script) => scriptsService.saveScript(script),
});
