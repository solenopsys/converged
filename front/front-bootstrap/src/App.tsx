...
import { ModuleConfig, LoadedModule, View, EventBusImpl } from "converged-core";
  ...
  useEffect(() => {
    const loadModules = async (): Promise<void> => {
      try {
          ...
          data.map(
            async (
              ...
              try {
                ...
                console.error(
                  `Failed to load module ${moduleConfig.name}:`,
                  error,
                );
                return null;
              }
        ...
        bus.run("daily_sales.mount_to_dashboard", {});
        bus.run("leads.mount_to_dashboard", {});
        bus.run("touches.mount_to_dashboard", {});
        ...
        const validModules = loadedModules.filter(
          (module): module is LoadedModule => module !== null,
        );
