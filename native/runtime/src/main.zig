//! runtime — the RT virtual machine service.
//!
...
pub fn main(init: std.process.Init) !void {
    ...
    std.debug.print("runtime: RT VM listening on {s} (backend={s}, scheduler={})\n", .{ bind, @tagName(store.backend), scheduler_on });
    ...
    while (true) {
        const stream = server.accept(io) catch |err| {
            std.debug.print("runtime: accept failed: {s}\n", .{@errorName(err)});
            continue;
        };
        ...
        handleConn(io, req_arena.allocator(), &engine, stream) catch |err| {
            std.debug.print("runtime: request failed: {s}\n", .{@errorName(err)});
        };
    }
}
