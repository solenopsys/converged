...
/// Load artifact path from current.json by target string
/// Returns the full path to the .a file in artifacts directory
pub fn loadArtifactPath(
    ...
    const hash = obj.get(target_str) orelse return error.TargetNotFound;

    return std.fmt.allocPrint(allocator, "{s}/{s}.so", .{ artifacts_dir, hash.string });
}

/// Load artifact hash from current.json by target string
pub fn loadArtifactHash(
    allocator: std.mem.Allocator,
    json_path: []const u8,
    target_str: []const u8,
) ![]const u8 {
...
/// Step that hashes the built executable and moves it to artifacts directory
pub const HashAndMoveExeStep = struct {
    ...
    pub fn create(
        ...
        const self = b.allocator.create(HashAndMoveExeStep) catch @panic("OOM");
        self.* = .{
            .step = std.Build.Step.init(.{
                .id = .custom,
                .name = "hash_and_move_exe",
                .owner = b,
                .makeFn = make,
            }),
            .exe_name = exe_name,
...
/// Helper to build an executable name with target suffix
