...
import { CacheStore, createCacheStore } from "./store";
...
export class CacheController {
	...
	constructor(private cacheDir: string) {
		this.cs = createCacheStore(`${cacheDir}/meta.json`);
	...
	async saveFile(
		...
		// Compress if needed
		const toSave = compress
			? await brotliCompressData(contentBuffer)
			: contentBuffer;
	...
	async readFile(
		hash: string,
	): Promise<{ buffer: ArrayBuffer; type: string; compressed: boolean }> {
