...
/** Chunks are stored compressed; rp-store records which codec was used.
 *  "deflate" here is fflate's raw deflate — the codec lm-compressors and
 *  rp-files write with — not zlib's wrapped form. */
...
async function convertWithCadAssistant(
	...
	const workDir = await mkdtemp(join(tmpdir(), "lm-modelconvertor-"));
...
async function convertWithAssimp(
	...
	const workDir = await mkdtemp(join(tmpdir(), "lm-modelconvertor-assimp-"));
	const inputPath = join(workDir, sourceName);
...
export class ModelConvertorServiceImpl implements ModelConvertorService {
	...
	private requiredCache(): CacheAdapter {
		if (!this.cache) {
			throw new Error("lm-modelconvertor requires the Valkey cache adapter");
		}
		return this.cache;
	}
	 ...
	 *
	 * Storage is chunked, so a file is several blobs in rp-store; only the keys
