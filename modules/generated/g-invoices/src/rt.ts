...
export interface InvoicesServiceRtClient {
  createInvoice(input: CreateInvoiceInput): InvoiceWithLines;
  getInvoice(id: InvoiceId): InvoiceWithLines | any;
  listInvoices(params: InvoiceListParams): PaginatedResult<Invoice>;
  issueInvoice(id: InvoiceId, dueAt?: ISODateString): Invoice;
  markPaid(id: InvoiceId, input: MarkPaidInput): boolean;
  voidInvoice(id: InvoiceId, reason?: string): Invoice;
  findByProviderRef(providerRef: string): Invoice | any;
  billedEntryIds(entryIds: string[]): string[];
}

export function createInvoicesServiceRtClient(): InvoicesServiceRtClient {
  return createRtClient<InvoicesServiceRtClient>(metadata);
}
