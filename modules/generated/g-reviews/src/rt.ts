...
export interface ReviewsServiceRtClient {
  ...
  listReviews(params: ReviewListParams): PaginatedResult<Review>;
  patchReview(id: ReviewId, patch: ReviewPatch): Review;
  setStatus(id: ReviewId, status: ReviewStatus): Review;
  replyToReview(id: ReviewId, reply: string): Review;
  deleteReview(id: ReviewId): boolean;
  describeSelection(objectType: string): SelectionDescriptor;
  inspectReviews(filter?: FilterObject): SelectionStats;
  getReviewsDashboard(): ReviewsDashboard;
  createInvite(input: ReviewInviteInput): ReviewInvite;
  listInvites(params: ReviewInviteListParams): PaginatedResult<ReviewInvite>;
  patchInvite(id: ReviewInviteId, patch: ReviewInvitePatch): void;
  findInvitesToFollowUp(days: number, maxFollowups: number, limit: number): ReviewInvite[];
  getInviteByToken(token: string): ReviewInviteView | any;
  markInviteOpened(token: string): ReviewInviteView | any;
  submitByToken(token: string, submission: ReviewSubmission): ReviewId | any;
  getSettings(): ReviewSettings;
  saveSettings(patch: ReviewSettingsPatch): ReviewSettings;
}
