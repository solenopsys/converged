...
// Client interface
export interface BusServiceClient {
  publish(event: BusEventInput): Promise<PublishResult>;
  subscribe(patterns: string[]): Promise<SubscribeResult>;
  unsubscribe(patterns: string[]): Promise<SubscribeResult>;
  replay(params: ReplayParams): Promise<ReplayResult>;
...
export function createBusServiceClient(
  config: CrullerTransportClientConfig,
): BusServiceClient {
  return createCrullerTransportClient<BusServiceClient>(metadata, config);
}

export function createBusServiceCrullerTransportClient(
  config: CrullerTransportClientConfig,
): BusServiceClient {
  return createBusServiceClient(config);
}
