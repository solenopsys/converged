...
function buildToolCallSummary(
	...
	return (
		[summary, title, status, modelId].filter(Boolean).join(" • ") || undefined
	);
}
...
function toChatMessage(
	...
	if (message.type !== MessageType.message && message.type !== "message")
		return null;
	...
	if (toolCall) {
		return {
			id:
				message.id ??
				`history-tool-${index}-${message.timestamp ?? Date.now()}`,
			beforeId: message.beforeId,
			type: user,
		...
		async function loadHistory() {
			...
			try {
				...
				const nextMessages = rows
					.map((message, index) =>
						toChatMessage(message, index, fileUploadedPrefix),
					)
