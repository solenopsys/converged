...
export const configFieldChanged = domain.createEvent<{
	field: string;
	value: any;
}>();
...
export const createNodeFx = domain.createEffect<NodeConfig, any>({
	handler: async (data) => {
		...
		const configResult = await dagClient.createNodeConfig(
			data.codeSource,
			data.config,
		);
...
export const $selectedCodeSource = domain
	.createStore("")
	.on(codeSourceSelected, (_, cs) => cs)
	.reset(formReset);

export const $schemaFields = domain
	.createStore<SchemaField[]>([])
	.on(fetchSchemaFx.doneData, (_, fields) => fields)
	.reset(formReset);

export const $configValues = domain
	.createStore<Record<string, any>>({})
	.on(configFieldChanged, (state, { field, value }) => ({
		...state,
		[field]: value,
	}))
	.reset(formReset)
			...
			<div key={field.name} className="space-y-2">
				...
				{isTextArea ? (
					<Textarea
						value={value}
						onChange={(e) =>
							configFieldChanged({ field: field.name, value: e.target.value })
						}
					...
					<Select
						...
						onValueChange={(v) =>
							configFieldChanged({ field: field.name, value: v === "true" })
						}
					>
						<SelectTrigger>
							<SelectValue />
			...
			{selectedCodeSource && (
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Configure node parameters for {selectedCodeSource}
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						{loading ? (
