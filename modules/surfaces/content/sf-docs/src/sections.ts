...
export async function loadDocsSections(
	indexPath: string,
	...
	return normalizeStructDocs(structData, indexPath).sort(
		(a, b) => a.order - b.order,
	);
}
...
function normalizeStructDocs(
	...
	for (let index = 0; index < data.length; index += 1) {
		...
		const slugFromId = rawId.split("/").pop()?.replace(/\.md$/i, "");
