import { cn, HeaderPanel, ThreadedChat } from "front-core";
import type {
	CommunitySection,
	CommunityTopic,
	SectionTreeNode,
...
function readCommunityEnv(): Required<CommunityEnv> {
	const globalEnv = (globalThis as any).__SF_ENV__ as
		| Record<string, unknown>
		| undefined;
	const raw = (globalEnv?.["sf-community"] ?? {}) as CommunityEnv;
...
export const CommunityView = () => {
	...
	const loadSections = useCallback(async () => {
		...
		setError(null);
		try {
			const tree = await communityClient.readSectionsTree(
				env.rootId || undefined,
				false,
			);
			...
			setSelectedSectionId((current) => {
				if (
					current &&
					flattenSections(nodes).some((node) => node.id === current)
				) {
					return current;
				}
	...
	const loadMessages = useCallback(
		async (topic: CommunityTopic | null, options?: { silent?: boolean }) => {
			...
			try {
				...
				const messagesRows = Array.isArray(rows)
					? (rows as ThreadMessage[])
					: [];
				...
				<div key={node.id} className={styles.treeNode}>
					<div className={styles.treeLine}>
						...
						<button
							...
							className={cn(
								styles.treeButton,
								isSelected ? styles.treeButtonActive : undefined,
							)}
							onClick={() => {
			...
			<div className={styles.layout}>
				<aside className={styles.sidebar}>
					<div className={styles.sidebarBody}>
						{isLoadingSections ? (
							<div className={styles.placeholder}>Loading sections...</div>
						) : null}
						{!isLoadingSections && sections.length === 0 ? (
							<div className={styles.placeholder}>No sections</div>
						) : null}
						<div className={styles.treeRoot}>
							{sections.map((node) => renderTreeNode(node))}
						</div>
					</div>
				</aside>
								...
								<ThreadedChat
									...
									renderMessage={(message) => {
										...
										return (
											<div
												className={cn(
													styles.messageRow,
													own ? styles.messageRowUser : styles.messageRowPeer,
												)}
											>
												<div
													className={
														own ? styles.userBubble : styles.peerBubble
													}
												>
													{!own ? (
														<div className={styles.peerUser}>
															{message.user}
														</div>
													) : null}
									...
									intro={
										<div className={styles.emptyState}>
											No posts yet. Be first to comment.
										</div>
									}
								/>
							</div>
