...
export const CallTranscriptView: React.FC<CallTranscriptViewProps> = ({
	sessionId,
}) => {
	const [transcript, setTranscript] = useState<GateTranscriptItem[]>([]);
	const [loading, setLoading] = useState(true);
	const [userAudioUrl, setUserAudioUrl] = useState<string | null>(null);
	const [assistantAudioUrl, setAssistantAudioUrl] = useState<string | null>(
		null,
	);
	...
	const loadData = async () => {
		...
		try {
			// Transcript and recordings both come from the rp-calls microservice —
			// never from the gate. rp-calls serves the transcript (read from
			// rp-threads) and builds the WebM/Opus recording on demand.
