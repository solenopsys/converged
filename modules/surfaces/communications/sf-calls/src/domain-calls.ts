...
export const refreshSessionsClicked = domain.createEvent(
	"REFRESH_SESSIONS_CLICKED",
);
export const sessionMetaRequested = domain.createEvent<string>(
	"SESSION_META_REQUESTED",
);
export const openCallDetail = domain.createEvent<{ sessionId: string }>(
	"OPEN_CALL_DETAIL",
);
...
export const loadSessionsFx = domain.createEffect({
	name: "LOAD_SESSIONS",
	// rp-calls is the scoped source of truth for persisted call sessions.
...
export const loadTranscriptFx = domain.createEffect({
	...
	handler: async (
		sessionId: string,
	): Promise<{ sessionId: string; items: GateTranscriptItem[] }> => ({
		sessionId,
...
export const loadSessionMetaFx = domain.createEffect({
	...
	handler: async (id: string): Promise<SessionMeta> => {
		// Audio presence comes from rp-calls (which owns the stored Opus frames),
...
export const $sessions = domain
	.createStore<string[]>([])
	// rp-calls already returns newest-first; keep its order.
...
export const $sessionMeta = domain
	...
	.on(loadSessionMetaFx.doneData, (state, meta) => ({
		...state,
		[meta.id]: meta,
	}))
