...
function readChatsEnv(): Required<ChatsEnv> {
	const globalEnv = (globalThis as any).__SF_ENV__ as
		...
		| undefined;
	const raw = (globalEnv?.["sf-chats"] ?? {}) as ChatsEnv;
...
const ChatView: React.FC = () => {
		...
		async (options?: { silent?: boolean }) => {
			...
			try {
				...
				const items = Array.isArray(response?.items)
					? (response.items as ChatRoom[])
					: [];
				setRooms(items);

				if (
					!selectedRoomId ||
					!items.some((room) => room.id === selectedRoomId)
				) {
					const preferred = env.roomId
						? items.find((room) => room.id === env.roomId)
						: undefined;
	...
	const loadThread = useCallback(
		async (options?: { silent?: boolean }) => {
			...
			try {
				const response = await threadsClient.readThreadAllVersions(
					selectedRoom.threadId,
				);
				const rows = Array.isArray(response)
					? (response as RawThreadMessage[])
					: [];
						...
						return (
							...
							>
								<div className={own ? styles.userBubble : styles.peerBubble}>
									{!own ? (
										<div className={styles.peerUser}>{message.user}</div>
									) : null}
