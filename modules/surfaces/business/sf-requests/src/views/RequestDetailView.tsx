...
import { $activeLocale, useSurfaceTranslation } from "front-core";
...
export function ManufacturingRequestPage({
	...
	const { t } = useSurfaceTranslation("sf-requests");
...
export function RequestDetailView({
	...
	useEffect(() => {
		...
		const refresh = async () => {
			try {
				const updated = await requestsClient.getRequestModel(activeRequestId);
				if (!cancelled && updated) {
					requestModelReceived(updated as RequestModel);
				}
			} catch {
				// The primary request loader owns the visible error state.
			}
		};
		void refresh();
