...
function SocialIcon({ provider }: { provider: OAuthProviderName }) {
	...
	if (provider === "microsoft") {
		return (
			...
			>
				<path
					d="M2 3h9v9H2V3zM13 3h9v9h-9V3zM2 14h9v9H2v-9zM13 14h9v9h-9v-9z"
					fill="currentColor"
				/>
			</svg>
		);
...
export function SocialsPanel() {
	const { t } = useSurfaceTranslation(AUTH_SF_ID);
	// Only providers the tenant actually configured are offered — the gateway
	// answers with the enabled ones, and an empty list renders no buttons rather
	// than dead ones.
	...
	useEffect(() => {
		...
		const loadTemplates = async () => {
			try {
				...
				setTemplates(
					response.map((item) => ({
						provider: item.provider as OAuthProviderName,
						displayName: normalizeProviderDisplayName(
							item.provider as OAuthProviderName,
							item.displayName,
						),
					})),
				);
