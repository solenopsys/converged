...
export class SessionManager {
  ...
  async createSession(model: string): Promise<SessionInfo> {
    const id = randomUUID();
		return this.ensureSession(id, model);
	}

	async ensureSession(sessionId: string, model: string): Promise<SessionInfo> {
		const existing = await this.stores.history.sessions.findById({ id: sessionId });
		if (existing) return this.toSessionInfo(existing);
  ...
  async appendMessage(
    ...
    await this.stores.history.sessions.update(
      { id: sessionId },
		{ updatedAt: now, messageCount: session.messageCount + 1 },
    );
