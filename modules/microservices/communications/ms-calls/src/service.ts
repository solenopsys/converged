...
export class CallsServiceImpl implements CallsService {
	...
	async getTranscript(id: CallId): Promise<CallTranscriptItem[]> {
		...
		void id;
		throw new Error(
			"[ms-calls] Direct service-to-service calls are forbidden. Centimanus must write the dialogue into ms-calls through saveDialogue.",
		);

		// The old direct ms-calls -> ms-threads HTTP call is intentionally disabled.
