...
class FakeProcessingStoreService {
	...
	getExecutionContext(
		workflowId: string,
		executionId: string,
	): { createdAt: string; meta?: any } | undefined {
...
class FakeStatsStoreService {
	...
	async updateNode(id: number, patch: any): Promise<NodeRow | undefined> {
		...
		if (patch.error_message !== undefined)
			row.errorMessage = patch.error_message;
...
describe("DagServiceImpl storage API with mocked stores", () => {
	...
	test("lists Solution-selected workflows without loading their source", async () => {
		const environment = {
			WORKFLOWS: process.env.WORKFLOWS,
			WORKFLOW_ENDPOINTS: process.env.WORKFLOW_ENDPOINTS,
			WORKFLOW_DIGESTS: process.env.WORKFLOW_DIGESTS,
			MODULE_PROXY: process.env.MODULE_PROXY,
		};
		try {
			process.env.WORKFLOWS = JSON.stringify([
				{
					id: "file-unpack",
					name: "wf-file-unpack",
			...
			process.env.WORKFLOW_ENDPOINTS = JSON.stringify({
				"workflows/wf-file-unpack.js":
					"http://127.0.0.1:9181/workflows%2Fwf-file-unpack.js",
			});
			process.env.WORKFLOW_DIGESTS = JSON.stringify({
				"workflows/wf-file-unpack.js": "sha256:unused-in-dev",
			});
			process.env.MODULE_PROXY = "http://ptah-proxy";

			await expect(createService().listAvailableWorkflows()).resolves.toEqual({
				items: [
					{
						id: "file-unpack",
						name: "wf-file-unpack",
						...
						sourceUrl: "http://127.0.0.1:9181/workflows%2Fwf-file-unpack.js",
					},
				],
			});
		} finally {
