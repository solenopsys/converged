...
export class CallsStoreService {
	...
	private requiredCache(): CacheAdapter {
		if (!this.cache) {
			throw new Error(
				"Valkey cache is required for rp-calls audio payload transfer",
			);
		}
		return this.cache;
	}
	...
	private async writeCacheRef(
		...
		const cacheKey = extra
			? cache.buildKey("rp-calls", kind, id, extra, crypto.randomUUID())
			: cache.buildKey("rp-calls", kind, id, crypto.randomUUID());
