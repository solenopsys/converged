import type { BusServiceClient } from "g-bus";
...
type Delivery = {
	...
	bus: Pick<BusServiceClient, "publish">;
	push: Pick<PushRouterServiceClient, "publish">;
	grant: (userId: string) => Promise<void>;
};

async function report(stage: string, requestId: string, action: () => Promise<unknown>) {
	try {
		await action();
	} catch (error) {
...
export async function announceRequestCreated(
	...
	await Promise.all([
		...
		report("bus", requestId, () =>
			delivery.bus.publish({
				name: `request.created.${requestId}`,
				source: "service",
				actor,
				dedupKey: `request.created:${requestId}`,
				payload: { requestId, source: input.source },
			}),
		),
	]);
	...
	try {
		...
		const limit = 100;
		while (true) {
			const page = await delivery.staff.listStaff({ offset, limit, active: true });
			for (const member of page.items) {
				const userId = member.userId?.trim();
				if (member.active && userId && userId !== actor) recipients.add(userId);
			}
			offset += page.items.length;
			if (page.items.length < limit || offset >= (page.totalCount ?? Infinity)) break;
		}
	} catch (error) {
	...
	await Promise.all([...recipients].map(async (userId) => {
		try {
			...
			console.warn("[rp-requests] staff access grant failed", requestId, userId, error);
			return;
		}
		await report("staff notification", requestId, () =>
			delivery.push.publish({
				...
				params: { id: requestId },
				link: { surface: "requests", ref: requestId, href: `/console/request/${encodeURIComponent(requestId)}` },
			}),
		);
	}));
}
