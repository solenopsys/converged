...
const outreachTargetFilterSchema: KyselyFilterSchema = {
	id: {
		valueType: "string",
		operators: ["eq", "notEq", "in", "notIn", "contains", "startsWith"],
		column: "outreach_targets.id",
	},
	outreachId: {
		valueType: "string",
		operators: ["eq", "notEq", "in", "notIn", "contains", "startsWith"],
		column: "outreach_targets.outreachId",
	},
	companyId: {
		valueType: "string",
		operators: ["eq", "notEq", "in", "notIn", "contains", "startsWith"],
		column: "outreach_targets.companyId",
	},
	templateId: {
		valueType: "string",
		operators: ["eq", "notEq", "in", "notIn", "contains", "startsWith"],
		column: "outreach_targets.templateId",
	},
	status: {
		valueType: "string",
		operators: ["eq", "notEq", "in", "notIn"],
		column: "outreach_targets.status",
	},
};
...
export class SalesStoreService {
		...
		const result = await this.store.db
			...
			.onConflict((oc) =>
				oc.column("id").doUpdateSet((eb) => ({
					...
					templateId: eb.ref("excluded.templateId"),
	...
	async listOutreachTargets(params: {
		...
		if (params.companyId) {
			itemsQuery = itemsQuery.where("companyId", "=", params.companyId);
			countQuery = countQuery.where("companyId", "=", params.companyId);
		}
		if (params.status) {
		...
		const [items, countRows] = await Promise.all([
			applyKyselyFilter(
				itemsQuery as any,
				params.filter,
				outreachTargetFilterSchema,
			).execute() as Promise<OutreachTargetEntity[]>,
			applyKyselyFilter(
				countQuery as any,
				params.filter,
				outreachTargetFilterSchema,
