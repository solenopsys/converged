...
import {
	...
	createAccessServiceClient,
} from "g-access/browser";
import {
	countGrants,
	type GrantTree,
	serializePermission,
...
function readPresetFile(target: string): {
	name: string;
...
const linkHandler: Handler = async (
	...
	const [userId, preset] = requireParam(
		param,
		"access preset link <userId> <preset>",
	);
	...
	console.log(
		`${userId}: ${BASE_PRESET} + ${preset} → ${countGrants(resolved)} grants`,
	);
};

const unlinkHandler: Handler = async (
	client: AccessServiceClient,
	_splitter,
	param,
) => {
	const [userId, preset] = requireParam(
		param,
		"access preset unlink <userId> <preset>",
	);
...
const showUserHandler: Handler = async (
	client: AccessServiceClient,
	_splitter,
	param,
) => {
...
const tagHandler: Handler = async (
	...
	const [userId, tag, mode] = requireParam(
		param,
		"access tag <userId> <tag> [mode]",
	);
	...
	console.log(
		"Takes effect in tokens issued from now on — a tag rides the JWT.",
	);
};

const untagHandler: Handler = async (
	client: AccessServiceClient,
	_splitter,
	param,
) => {
	...
	protected initializeCommandMap(): Map<string, CommandEntry> {
		return new Map([
			[
				"preset",
				{
					handler: presetRouter,
					description:
						"Roles as files: access preset list | show <name> | set <name|path> | seed | link <userId> <preset> | unlink <userId> <preset>",
				},
			],
			[
				"user",
				{
					...
					description: "What one user actually holds: access user <userId>",
				},
			],
			[
				"tag",
				{
					handler: tagHandler,
					description:
						"Put a user in a group: access tag <userId> <tag> [mode]",
				},
			],
			[
				"untag",
				{
					handler: untagHandler,
					description:
						"Take a user out of a group: access untag <userId> <tag>",
				},
			],
		]);
	}
...
export default () => {
	const client: AccessServiceClient = createAccessServiceClient(
