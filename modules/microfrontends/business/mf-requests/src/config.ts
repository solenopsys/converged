...
export const createRequestsColumns = (t: Translate) => [
	...
	},
	{
		...
		statusConfig: {
			new: {
				label: t("list.status.new"),
				className: "bg-blue-100 text-blue-800",
			},
			draft: {
				label: t("list.status.draft"),
				className: "bg-gray-100 text-gray-700",
			},
			needs_clarification: {
				label: t("list.status.needs_clarification"),
				className: "bg-yellow-100 text-yellow-800",
			},
			ready: {
				label: t("list.status.ready"),
				className: "bg-green-100 text-green-800",
			},
			in_production: {
				label: t("list.status.in_production"),
				className: "bg-purple-100 text-purple-800",
			},
			done: {
				label: t("list.status.done"),
	...
	{
		id: "completion",
		title: t("list.columns.completion"),
		type: COLUMN_TYPES.TEXT,
		width: 100,
