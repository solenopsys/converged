...
const galeryClient = createGaleryServiceClient(createFrontNrpcClientConfig());
...
async function fetchAllRows(): Promise<GaleryListRow[]> {
	...
	while (true) {
		const page = await galeryClient.listGaleries({
		...
		const total =
			typeof page?.totalCount === "number" ? page.totalCount : galleries.length;
...
async function listOfGaleryItems(
	params: PaginationParams,
): Promise<{ items: GaleryListRow[]; totalCount: number }> {
