...
import { $registeredCommands, invokeAction } from "front-core";
...
export const functionsOpened = domain.createEvent("FUNCTIONS_OPENED");
export const functionsClosed = domain.createEvent("FUNCTIONS_CLOSED");
export const $screen = domain
  .createStore<"closed" | "list">("closed", { name: "SCREEN" })
  .on(functionsOpened, () => "list")
  .on(functionsClosed, () => "closed");
...
const executeCommandFx = domain.createEffect<{ id: string; params?: any }, unknown>({
  name: "EXECUTE_COMMAND",
  handler: ({ id, params }) => invokeAction(id, params ?? {}),
});
