...
export const refreshCommandsClicked = domain.createEvent(
	"REFRESH_COMMANDS_CLICKED",
);
export const executeCommandClicked = domain.createEvent<{
	commandId: string;
	params?: any;
}>("EXECUTE_COMMAND_CLICKED");

export const $commandsFilter = domain.createStore<string>("", {
	name: "COMMANDS_FILTER",
});
export const setCommandsFilter = domain.createEvent<string>(
	"SET_COMMANDS_FILTER",
);
$commandsFilter.on(setCommandsFilter, (_, filter) => filter);

export const $filteredCommands = $registeredCommands.map(
	(commands) => commands,
);

export const executeCommandFx = domain.createEffect<
