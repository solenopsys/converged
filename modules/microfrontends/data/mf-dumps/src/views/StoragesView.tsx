...
const bulkActions = [{ id: "dump", label: "Dump", icon: Archive }];
...
export const StoragesView = ({ bus }: { bus: any }) => {
	...
	const openStorageDashboard = useCallback(
		(row: { name: string; size: number }) => {
	...
	const handleBulkAction = useCallback(
		async (actionId: string, rows: any[]) => {
			for (const row of rows) {
				if (actionId === "dump")
					await (dumpsService as any).createDump(row.name);
			}
			refreshStorageStats();
		},
		[],
