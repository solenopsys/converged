...
import { Button, Input, Label, Plus, Save, Trash2 } from "front-core";
...
export const SecretDetailView = () => {
	...
	useEffect(() => {
		if (current?.data) {
			setEntries(
				Object.entries(current.data).map(([key, value]) => ({ key, value })),
			);
		}
	}, [current]);
	...
	if (!current) {
		return (
			<div className="p-6 text-muted-foreground">Select a secret to view</div>
		);
	}
