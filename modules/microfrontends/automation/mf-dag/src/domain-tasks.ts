...
export const openTasksList = domain.createEvent<{ executionId: string }>(
	"OPEN_TASKS_LIST",
);
...
export const $currentExecutionId = domain
...
export const loadTasksFx = domain.createEffect<string | null, Task[]>({
	handler: async (executionId) => {
		const result = await dagService.listTasks(executionId, {
			offset: 0,
			limit: 100,
		});
		return result.items;
