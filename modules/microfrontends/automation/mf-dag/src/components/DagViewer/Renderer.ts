...
export type NodeTypeProvider = (nodeName: string) => Promise<string>;
...
export class DAGRenderer {
	...
	getTotalHeight(nodeCount: number): number {
		...
		return (
			theme.topOffset + (nodeCount - 1) * theme.cellSize + theme.cellSize / 2
		);
	}
	...
	private async renderIcon(
		...
		try {
			const image = await this.loadIcon(
				iconName,
				this.getColor(theme.colors.nodeText),
			);
			this.ctx.drawImage(
				image,
				roundedX - size / 2,
	...
	private renderConnections(state: RenderState): void {
		state.edges.forEach(([fromName, toName]) => {
			...
			const controlOffset =
				theme.bezier.baseOffset +
				(nodeDistance - 1) * theme.bezier.distanceMultiplier;
