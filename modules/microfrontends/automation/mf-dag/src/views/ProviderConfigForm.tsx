...
export const providerConfigFieldChanged = domain.createEvent<{
	field: string;
	value: any;
}>();
...
export const $providerName = domain
	.createStore("")
	.on(providerNameChanged, (_, name) => name)
	.reset(providerFormReset);

export const $providerSelectedCodeSource = domain
	.createStore("")
	.on(providerCodeSourceSelected, (_, cs) => cs)
	.reset(providerFormReset);

export const $providerSchemaFields = domain
	.createStore<SchemaField[]>([])
	.on(fetchProviderSchemaFx.doneData, (_, fields) => fields)
	.reset(providerFormReset);

export const $providerConfigValues = domain
	.createStore<Record<string, any>>({})
	.on(providerConfigFieldChanged, (state, { field, value }) => ({
		...state,
		[field]: value,
	}))
	.reset(providerFormReset)
...
export const $providerLoading = domain
	.createStore(false)
...
sample({
	...
	source: {
		name: $providerName,
		codeSource: $providerSelectedCodeSource,
			...
			{selectedCodeSource && (
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Configure provider parameters for {selectedCodeSource}
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						{loading ? (
