...
// Client interface
export interface RuntimeAgentServiceClient {
  createSession(model?: string): Promise<SessionInfo>;
  sendMessage(sessionId: string, content: string): AsyncIterable<AgentStreamEvent>;
  getSession(sessionId: string): Promise<SessionInfo>;
  listSessions(params: PaginationParams): Promise<PaginatedResult<SessionInfo>>;
...
// Factory function
export function createRuntimeAgentServiceClient(
  config?: { baseUrl?: string },
): RuntimeAgentServiceClient {
  return createHttpClient<RuntimeAgentServiceClient>(metadata, config);
}
