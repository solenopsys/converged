...
export interface RuntimeAssistantServiceRtClient {
  createSession(serviceType?: ServiceType, model?: string, contextName?: string, language?: string): string;
  sendMessage(sessionId: string, messages: ContentBlock[], options?: ConversationOptions): StreamEvent;
  subscribeToRequestModel(requestId: string): RequestModelSnapshot;
...
export function createRuntimeAssistantServiceRtClient(): RuntimeAssistantServiceRtClient {
  return createRtClient<RuntimeAssistantServiceRtClient>(metadata);
}
