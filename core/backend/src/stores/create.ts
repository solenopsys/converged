...
export class StorageConnectionPool {
	...
	addConnection(config: StorageConnectionTarget): StorageConnection {
		...
		const normalized = typeof config === "string" ? { kind: "unix" as const, socketPath: config } : config;
		// fujin-routed storage has no fd to hand to the native pool (behemoth is a
		// DEALER peer, not a listener) — StorageConnection owns its own messaging
		// connection directly. Only unix/tcp direct sockets go through the pool.
		let conn: StorageConnection;
		if (normalized.kind === "fujin") {
			conn = new StorageConnection(normalized, this.options);
		} else {
			this.nativePool.add(key, normalized);
