...
type TenantStorageEndpoint =
	...
	| { host: string; port?: number; target?: string; service?: string; cacheHost?: string; cachePort?: number };
...
export function parseStorageHost(
	...
	// Network endpoints name a tenant's fujin ZMQ ROUTER, not a direct storage
	// socket — behemoth is a fujin DEALER peer (see cluster-messaging.md), so
	// every storage op is routed through fujin to the "behemoth"/"storage" peer.
	...
	return { kind: "fujin", host: tcpHost, port };
}
...
function storageEndpointToTarget(
	...
	const target = parseStorageHost(endpoint.host, endpoint.port);
	return target.kind === "fujin"
		? { ...target, target: endpoint.target, service: endpoint.service }
		: target;
}

export function resolveStorageCacheEndpointForScope(scope?: string): { host: string; port?: number } {
	const tenantServices = settings.storage.tenantServices<Record<string, TenantStorageEndpoint>>();
	const effectiveScope = scope ?? settings.storage.explicitScope();
	if (!effectiveScope) throw new SettingsError("Storage scope is required for cache routing");
	const endpoint = tenantServices[effectiveScope];
	if (!endpoint) throw new SettingsError(`No storage endpoint for scope "${effectiveScope}" in STORAGE_TENANT_SERVICES`);
	if (typeof endpoint === "string" || "kind" in endpoint) {
		const target = storageEndpointToTarget(endpoint);
		if (typeof target === "string" || target.kind === "unix") throw new SettingsError("Valkey cache requires a network storage endpoint");
		return { host: target.host };
	}
	return { host: endpoint.cacheHost ?? endpoint.host, port: endpoint.cachePort };
}
