...
function storageCacheUrl(config: RuntimeCacheConfig): string {
	...
	const endpoint = resolveStorageCacheEndpointForScope(scope);
	return `redis://${endpoint.host}:${endpoint.port ?? settings.cache.valkeyPort()}`;
}
...
export function createBunRedisCache(config: RuntimeCacheConfig): CacheAdapter {
	...
	const get = async (key: string): Promise<string | null> => {
		const value = await withClient((redis) => redis.get(key));
	...
	const getBytes = async (key: string): Promise<Uint8Array | null> => {
		const value = await withClient((redis) => redis.getBuffer(key));
