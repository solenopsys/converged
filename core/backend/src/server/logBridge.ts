...
export function installBackendLogBridge(options: BackendLogBridgeOptions) {
	...
	// Log events are discarded. Shipping them anywhere means a network call on
	// the error path, and an error on that path produces another event — the
	// bridge then feeds itself and drowns the container in retries. Console
	// output stays untouched; collection belongs outside the process.
	const postEvent = async (_event: LogEventInput): Promise<void> => {};
	...
	process.on("unhandledRejection", (reason) => {
		originalConsoleError("[runtime] unhandledRejection:", reason);
		enqueue({
			level: LEVEL_ERROR,
	...
	process.on("uncaughtException", (error) => {
		originalConsoleError("[runtime] uncaughtException:", error);
		enqueue({
			level: LEVEL_ERROR,
