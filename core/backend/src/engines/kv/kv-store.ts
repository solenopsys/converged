...
import { type Migration, Migrator } from "../../migrations";
...
export class KVStore implements KVStoreIntf, Store {
	...
	/**
	 * Store a value the writer already left in Valkey, naming it rather than
	 * sending it. Storage reads the cache entry itself, so the bytes never cross
	 * the transport — the point of the call is that the payload stays put.
	 *
	 * The cached bytes must already be in this store's wire format: whoever wrote
	 * them owns their framing, because nothing here re-encodes them.
	 */
	putFromCache(chain: string[], cacheKey: string): string {
		const key = chain.join(KEY_SEPARATOR);
		this.conn.kvPutFromCache(this.ms, this.storeName, key, cacheKey);
		return key;
	}
