...
const token = (await Bun.file(
	...
	.trim();
if (!token) throw new Error("RT_SERVICE_TOKEN not found in converged-local.env");

for (const name of WORKFLOWS) {
	const js = await buildWorkflow(join(import.meta.dir, "workflows", name, "index.ts"));
	...
	if (!res.ok) {
		throw new Error(`saveScript ${name} failed: ${res.status} ${await res.text()}`);
	}
