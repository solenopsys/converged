...
export type { WorkspaceContext } from "./workspace-context-registry";
...
type ALS = {
	...
	enterWith(store: WorkspaceContext): void;
};

const WORKSPACE_CONTEXT_STORAGE_KEY =
	"__CONVERGED_NRPC_WORKSPACE_CONTEXT_STORAGE__";
const runtimeGlobal = globalThis as typeof globalThis & {
	[WORKSPACE_CONTEXT_STORAGE_KEY]?: ALS;
};
...
function getStorage(): ALS | null {
	...
	if (runtimeGlobal[WORKSPACE_CONTEXT_STORAGE_KEY]) {
		_storage = runtimeGlobal[WORKSPACE_CONTEXT_STORAGE_KEY];
	...
	try {
		...
		if (mod?.AsyncLocalStorage) {
			...
			runtimeGlobal[WORKSPACE_CONTEXT_STORAGE_KEY] = _storage;
			setWorkspaceContextResolver(() => _storage?.getStore());
		}
	} catch {
