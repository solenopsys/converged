...
describe("chat messages", () => {
	...
	test("follows the shell locale and translates tool-call chrome", () => {
		initChatMessages(undefined, "en");

		LocaleController.getInstance().setLocale("ru");

		expect(translate("chat", "toolCall.kind")).toBe("Функция");
		expect(translate("chat", "toolCall.statusCompleted")).toBe("Готово");
	});
});
