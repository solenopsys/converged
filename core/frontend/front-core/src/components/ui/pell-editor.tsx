...
export type PellEditorProps = Omit<React.HTMLAttributes<HTMLDivElement>, "children" | "onChange"> & {
	value?: string;
	defaultValue?: string;
...
function getContentNode(root: HTMLElement | null): HTMLElement | null {
	return (root as (HTMLElement & { content?: HTMLElement }) | null)?.content ?? null;
}
...
function PellEditor({
	...
	React.useEffect(() => {
		...
		root?.querySelectorAll<HTMLButtonElement>(".pell-editor__button").forEach((button) => {
