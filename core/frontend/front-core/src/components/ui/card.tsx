...
import { dashboardSlots, subscribeDashboardIndicators } from "../../dashboard/dashboard-slots";
...
type ResolvedDashboardPin = Required<Pick<DashboardPinMeta, "id">> & Omit<DashboardPinMeta, "id">;
...
function DashboardPinnableCard({
	...
	const [pinned, setPinned] = React.useState(() => dashboardSlots.isPinned(pinId));
	...
	}, [pinId, dashboardPin.title, dashboardPin.source, dashboardPin.componentKey, dashboardPin.position]);
...
function Card({ className, dashboardPin, pinnable, children, ...props }: CardProps) {
	...
	return (
		<DashboardPinnableCard className={className} dashboardPin={resolvedPin} {...props}>
			{children}
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			className={cn("flex items-center px-4 pt-2 shrink-0 [.border-t]:pt-4", className)}
			{...props}
		/>
	);
}
