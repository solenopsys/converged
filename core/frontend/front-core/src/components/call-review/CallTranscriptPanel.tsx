...
import { Bot, MessageSquare, MicOff, RefreshCw } from "../../icons";
import { Badge } from "../ui/badge";
import { StereoCallPlayer } from "../../audio";
		...
		<div className={`flex h-full min-h-0 flex-col ${className ?? ""}`}>
			<div className="flex items-center gap-2 px-4 h-12 border-b border-border shrink-0">
				...
				<div className="flex-1 min-w-0">
					...
					<code className="text-[11px] text-muted-foreground font-mono">{callSessionLabel(sessionId)}</code>
				</div>
				{onRefresh && (
					<button
						onClick={onRefresh}
		...
		<div className={`flex gap-2 text-sm ${isUser ? "justify-end" : "justify-start"}`}>
			<div
				className={`max-w-[88%] rounded-2xl px-3 py-2 ${
					isUser ? "bg-blue-900/40 text-blue-100 rounded-br-sm" : "bg-muted text-foreground rounded-bl-sm"
				}`}
			>
