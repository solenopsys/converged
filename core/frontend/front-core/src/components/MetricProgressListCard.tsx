...
import { Card, CardContent, CardDescription, CardHeader, type DashboardPinMeta } from "./ui/card";
...
export function MetricProgressListCard({
	...
	return (
		<Card className={cn("flex h-full min-h-[320px] flex-col py-4", className)} dashboardPin={dashboardPin}>
			<CardHeader className="shrink-0 px-4 pb-2">
				<CardDescription>{title}</CardDescription>
				{description ? <div className="text-xs text-muted-foreground">{description}</div> : null}
			</CardHeader>
			<CardContent className={cn("min-h-0 flex-1 overflow-auto px-4 pb-4 pt-0", contentClassName)}>
				{items.length === 0 ? (
						...
						{items.map((item) => {
							...
							return (
								...
								>
									...
									<div className="text-right font-mono text-xs">{item.value ?? ""}</div>
									...
									<div className="flex min-w-0 flex-wrap gap-x-3 gap-y-1 text-[11px] text-muted-foreground">
										{(item.metrics ?? []).map((metric) => (
											<span key={metric.key} className="inline-flex items-center gap-1" title={metric.label}>
												{metric.color ? (
													<span className="h-2 w-2 rounded-[2px]" style={{ backgroundColor: metric.color }} />
												) : null}
												<span>{metric.label}</span>
												<span className="font-mono text-foreground">{formatMetricValue(metric.value)}</span>
											</span>
										))}
									</div>
								</div>
