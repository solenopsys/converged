...
import type { HeaderAction, HeaderPanelConfig, SelectionAction } from "../components/HeaderPanel";
...
import type { BulkAction, ColumnConfig, RowCardProps, RowId, ViewMode } from "../table/types";
import { TableFilterBar, type TableFilterConfig, type TableFilterValues } from "../table/TableFilterBar";
...
export interface EntityListViewProps<TData extends object = Record<string, unknown>> {
	tableId: string;
	store?: InfiniteTableStore;
	columns: Array<ColumnConfig<TData>>;
	title?: string;
...
export function EntityListView<TData extends object = Record<string, unknown>>({
	...
	const internalTabState = useMemo(() => {
		...
		const $active = createStore(tabs?.[0]?.id ?? "").on(changed, (_, value) => value);
	...
	if (!activeStore) {
		throw new Error(`EntityListView(${tableId}): no store — pass "store" or per-tab "store"`);
	}
	...
	const mergedFilters = useMemo(
		() => cleanFilters({ ...baseFilters, ...activeTab?.filters, ...filterValues }),
		...
		<HeaderPanelLayout config={headerConfig} className={className}>
			<div className="flex h-full min-h-0 flex-col gap-3">
				{filters && filters.length > 0 && (
					<TableFilterBar filters={filters} values={filterValues} onChange={setFilterValues} />
				)}
