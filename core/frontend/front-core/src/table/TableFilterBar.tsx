...
function SearchFilter({
	...
	const commit = (next: string) => {
		setDraft(next);
		if (timerRef.current) clearTimeout(timerRef.current);
		timerRef.current = setTimeout(() => onValueChange(next), filter.debounceMs ?? 400);
	};
	...
	return (
		<div className={cn("flex flex-wrap items-center gap-2", className)}>
			{filters.map((filter) => {
				...
				return (
					<Select key={filter.id} value={String(values[filter.id] ?? ALL_VALUE)} onValueChange={(value) => setValue(filter.id, value)}>
						<SelectTrigger size="sm" className={cn("w-44", filter.className)}>
							<SelectValue placeholder={filter.label ?? filter.id} />
						</SelectTrigger>
						<SelectContent>
							<SelectItem value={ALL_VALUE}>{filter.allLabel ?? filter.label ?? "All"}</SelectItem>
