...
export const findElements = () => {
	...
	elements.left.content = document.querySelector(
		`[${ATTR.contentContainer}="left"]`,
	);
	...
	elements.right.content = document.querySelector(
		`[${ATTR.contentContainer}="right"]`,
	);
	elements.right.tabs = document.querySelector(
		`[${ATTR.tabsContainer}="right"]`,
	);
};
...
const bindStoreToDOM = () => {
	const unwatchLeft = $leftSidebarState.watch((state) =>
		applyState("left", state),
	);
	const unwatchRight = $rightSidebarState.watch((state) =>
		applyState("right", state),
	);
	const unwatchLeftWidth = $leftSidebarWidth.watch((width) =>
		applyWidth("left", width),
	);
	const unwatchRightWidth = $rightSidebarWidth.watch((width) =>
		applyWidth("right", width),
	);
