...
extern "C" TransportRequest* transport_req_file_get_to_cache(const char* ms, const char* store, const char* key) {
    try {
        auto* r = make_req(ms, store);
        if (!r) return nullptr;
        r->req.getBody().initFileGetToCache().setKey(key);
        return r;
    } TRANSPORT_CATCH_RET(nullptr, "transport_req_file_get_to_cache")
}
...
extern "C" TransportRequest* transport_req_kv_compact(const char* ms, const char* store) {
    try {
        auto* r = make_req(ms, store);
        if (!r) return nullptr;
        r->req.getBody().setKvCompact();
        return r;
...
extern "C" const char* transport_req_reader_key(TransportRequestReader* r) {
    ...
    try {
        ...
        if (which == Request::Body::FILE_GET_TO_CACHE) return r->req.getBody().getFileGetToCache().getKey().cStr();
