...
test "EdDSA JWT verifies claims and preserves trusted context" {
    ...
    const config = Config{ .issuer = "ms-access", .audience = "cluster", .keys = &keys };
    const token = try makeToken(
        std.testing.allocator,
        key_pair,
        "{\"alg\":\"EdDSA\",\"kid\":\"current\"}",
        "{\"typ\":\"user\",\"sub\":\"admin\",\"scope\":\"club\",\"perm\":[\"fujin/state(r)\"],\"iat\":100,\"exp\":200,\"iss\":\"ms-access\",\"aud\":\"cluster\"}",
    );
...
test "EdDSA JWT rejects tampering and invalid mandatory claims" {
    ...
    const token = try makeToken(
        ...
        key_pair,
        "{\"alg\":\"EdDSA\",\"kid\":\"current\"}",
        "{\"typ\":\"user\",\"sub\":\"admin\",\"scope\":\"club\",\"perm\":[],\"iat\":100,\"exp\":200,\"iss\":\"ms-access\",\"aud\":\"cluster\"}",
    );
    defer std.testing.allocator.free(token);
    ...
    try std.testing.expectError(error.AudienceRejected, verify(std.testing.allocator, token, .{ .issuer = "ms-access", .audience = "other", .keys = &keys }, 150));
}
