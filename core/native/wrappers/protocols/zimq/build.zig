...
pub fn build(b: *std.Build) void {
    ...
    const build_all = b.option(bool, "all", "Build all supported Linux targets") orelse false;

    if (build_all) {
        const hashes = build_utils.createHashMap(b);
        const json_step = build_utils.WriteJsonStep.create(b, hashes, current_json);
        var previous_hash_step: ?*std.Build.Step = null;

        for (build_utils.supported_targets) |query| {
            const resolved = b.resolveTargetQuery(query);
            const target_str = build_utils.getTargetString(resolved);
            const lib_name = build_utils.getLibName(std.heap.page_allocator, "zimq", target_str);
            const built = buildLibzmq(b, resolved, optimize, strip, options, lib_name);
            const install = b.addInstallArtifact(built.artifact, .{});
            ...
            // HashAndMoveStep mutates one shared manifest map. Keep those four
            // short copy/hash operations ordered while target compilation can
            // still proceed independently.
            if (previous_hash_step) |previous| hash_step.step.dependOn(previous);
            previous_hash_step = &hash_step.step;
        }

        if (previous_hash_step) |last| json_step.step.dependOn(last);

        b.default_step.dependOn(&json_step.step);
        return;
    }

    const built = buildLibzmq(b, target, optimize, strip, options, "zimq");
    b.installArtifact(built.artifact);
    const zimq = addZigBinding(b, target, optimize, strip, curve, built.header);
...
fn addZigBinding(
    ...
    strip: bool,
    curve: bool,
    zmq: *std.Build.Module,
) *std.Build.Module {
    const zimq = b.addModule("zimq", .{
...
const BuiltLibzmq = struct {
    artifact: *std.Build.Step.Compile,
    header: *std.Build.Module,
};

/// Compile the pinned libzmq source tree into a shared library. The exported
/// API is the upstream C ABI (`zmq_*`); no system libzmq is linked at runtime.
fn buildLibzmq(
    b: *Build,
    target: Build.ResolvedTarget,
    optimize: std.builtin.OptimizeMode,
    ...
    // libzmq is C++ and pulls in a static libc++. Without this script those
    // instantiations are exported and interpose the libc++ inside any library
    // dlopen'ed later in the same process (resonus + libdatachannel), which
    // corrupts the heap. See zmq.map.
    library.setVersionScript(b.path("zmq.map"));
    ...
    return .{ .artifact = library, .header = header };
}
