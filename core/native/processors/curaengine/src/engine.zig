//! CuraEngine over its C wrapper (`wrappers/slicers/curaengine`).
//!
//! Everything specific to slicing lives here: the FFI structs, the task
//! schema, and the defaults. The shared package knows none of it — it only
//! ever sees task JSON in and result JSON out.
...
pub const CuraEngine = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, library_path: []const u8) !CuraEngine {
        return .{
            .allocator = allocator,
    ...
    pub fn deinit(self: *CuraEngine) void {
        self.stop();
        self.allocator.free(self.library_path);
        self.* = undefined;
    }

    pub fn processor(self: *CuraEngine) Processor {
        return .{
            .name = "curaengine",
            .ctx = self,
    ...
    fn stop(self: *CuraEngine) void {
        _ = self;
        // Wrapper libraries carry C/C++ static runtime state. Keeping a
        // successfully loaded image mapped until process exit avoids dlclose
        // invalidating a runtime atexit/TLS handler. Releasing the whole
        // process is the operator's job now, and it is a clean one.
    }

    fn execute(self: *CuraEngine, allocator: std.mem.Allocator, request_json: []const u8, progress: ?Progress) ![]u8 {
        ...
        emitPhase(progress, "reading", model_name);
        ...
        emitPhase(progress, "slicing", model_name);
    ...
    fn startOpaque(ctx: *anyopaque) !void {
        const self: *CuraEngine = @ptrCast(@alignCast(ctx));
        try self.start();
    }
...
/// Best-effort progress marker. The wrapper is one blocking FFI call with no
/// layer callback yet, so we bracket it with coarse phases; when the wrapper
/// exposes per-layer progress this is where it threads through.
fn emitPhase(progress: ?Progress, phase: []const u8, model: []const u8) void {
    const sink = progress orelse return;
    var buffer: [512]u8 = undefined;
    const event = std.fmt.bufPrint(
        &buffer,
        "{{\"phase\":\"{s}\",\"processor\":\"curaengine\",\"model\":\"{s}\"}}",
        .{ phase, model },
    ) catch return;
