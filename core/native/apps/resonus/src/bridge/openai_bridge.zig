...
pub const SessionKind = enum {
    ...
    /// (instructions/voice/model unused; empty transcription_languages = auto).
    transcription,
};
...
pub const Config = struct {
    ...
    /// Comma-separated ISO codes for GPT-Transcribe `languages`; empty = auto.
    transcription_languages: []const u8,
...
pub const BaseConfig = struct {
    ...
    /// `languages` is the context's language field: one ISO code, or a
    ...
    pub fn withContext(self: BaseConfig, instructions: []const u8, languages: []const u8) Config {
        return .{
            ...
            .transcription_keywords = self.transcription_keywords,
...
pub const OpenAIBridge = struct {
    ...
    pub fn sendAudio(self: *OpenAIBridge, data: []const u8) void {
        ...
        // Media MUST go out as a binary frame: the RTP header alone carries zero
        // bytes (timestamp/SSRC), so the text path would cut every packet down to
        // a few bytes and libdatachannel would reject all of them (rc=-2) — the
        // model then hears nothing at all from the caller.
        self.dc.sendBinary(self.track, packet) catch |err| {
            if (seq < 5) {
    ...
    pub fn sendEvent(self: *OpenAIBridge, json: []const u8) void {
        ...
        self.dc.sendText(self.dc_id, json) catch |err| {
    ...
    fn sendResponseCreate(self: *OpenAIBridge) !void {
        ...
        try self.dc.sendText(self.dc_id, msg);
    }
