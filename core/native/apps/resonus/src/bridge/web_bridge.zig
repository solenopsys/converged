...
pub const WebBridgeSession = struct {
    ...
    /// The caller asked to hang up over signaling. Only flags the session: the
    /// actual teardown runs on the reaper thread, never on the signaling one.
    pub fn requestHangup(self: *WebBridgeSession) void {
        std.log.info("web-bridge [{s}]: hangup requested, handing over to reaper", .{self.session_id});
    ...
    fn onOpenAIAudio(audio: []const u8, ctx: ?*anyopaque) void {
        ...
        //
        // Both branches send BINARY: the text path derives the length from the
        // first zero byte, which chops an Opus frame at a random offset — that is
        // what made forwarded audio sound broken/clipped.
        ...
        if (hdr != null and hdr.?.payload_type != self.browser_opus_pt) {
            ...
            if (audio.len <= buf.len) {
                ...
                self.dc.sendBinary(self.browser_track, buf[0..audio.len]) catch {};
                return;
            }
        }
        self.dc.sendBinary(self.browser_track, audio) catch {};
    }
