...
pub const HttpServer = struct {
    ...
    pub fn serve(self: *HttpServer) !void {
        ...
        std.log.info("resonus listening on {s}:{d}", .{ self.cfg.http_host, self.cfg.http_port });
    ...
    fn serveCompatWebSocket(
        ...
        defer {
            if (active_session_id) |sid| {
                // Flag it; the reaper ends the session off this thread.
                _ = self.gateway.requestWebSessionHangup(sid);
    ...
    fn handleCompatWebSocketText(
        ...
        if (std.mem.eql(u8, msg_type, "offer")) {
            ...
            if (active_session_id.*) |old| {
                _ = self.gateway.requestWebSessionHangup(old);
