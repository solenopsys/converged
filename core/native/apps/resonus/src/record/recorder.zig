...
/// Upper bound for flushPending(). Draining is best-effort by design: the call
/// teardown path runs here, and it must return even if something is still
/// feeding the queue.
const FLUSH_DEADLINE_NS: i64 = 5 * std.time.ns_per_s;
...
const RecorderState = struct {
    ...
    fn queuedCount(self: *RecorderState) usize {
        self.mutex.lock();
        defer self.mutex.unlock();
        return self.queue.items.len;
    }

    /// Write out everything queued, then return. Bounded: callers reach this from
    /// the call-teardown path, and an audio callback still enqueueing frames
    /// (~50/s per direction) would otherwise keep the queue non-empty forever and
    /// block the caller indefinitely.
    fn flushPending(self: *RecorderState) void {
        const deadline = clock.nanoTimestamp() + FLUSH_DEADLINE_NS;
        while (true) {
            ...
            if (clock.nanoTimestamp() >= deadline) {
                std.log.warn("recorder: flush deadline reached, {d} frame(s) left to the worker", .{self.queuedCount()});
                return;
            }
            clock.sleepMs(1);
        }
