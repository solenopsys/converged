...
pub const Config = struct {
    ...
    openai_model: []u8,
    openai_voice: []u8,
    // GPT-Transcribe family: `languages`, `prompt` and `keywords` instead of
    // Whisper's single `language`.
    //
    // Both legs run `gpt-transcribe`. `gpt-live-transcribe` is NOT usable here:
    // POST /v1/realtime/calls hangs on it and returns 504 after ~15s (verified
    // for every field combination, including no language at all), so a session
    // built on it never opens. Legacy models are equally unusable now — they
    // 504 the same way as soon as `languages` is sent.
    openai_transcription_model: []u8,
    openai_transcription_prompt: ?[]u8,
    openai_transcription_keywords: ?[]u8,
    dictation_transcription_model: []u8,
    dictation_transcription_prompt: ?[]u8,
    dictation_transcription_keywords: ?[]u8,
    ...
    fujin_endpoint: []u8,
    fujin_client_target: []u8,
    /// Identity for Resonus' own calls to the services target. This is never a
    /// browser token: calls, context lookup and recording run server-to-server.
    service_token: []u8,
    valkey_url: []u8,
    ...
    pub fn init(allocator: std.mem.Allocator, environ: *const std.process.Environ.Map) !Config {
        var cfg = Config{
            ...
            .openai_transcription_model = try envOwnedOrDefault(allocator, environ, "OPENAI_REALTIME_TRANSCRIPTION_MODEL", "gpt-transcribe"),
            .openai_transcription_prompt = try envOwnedOptionalTrimmed(allocator, environ, "OPENAI_REALTIME_TRANSCRIPTION_PROMPT"),
            .openai_transcription_keywords = try envOwnedOptionalTrimmed(allocator, environ, "OPENAI_REALTIME_TRANSCRIPTION_KEYWORDS"),
            .dictation_transcription_model = try envOwnedOrDefault(allocator, environ, "OPENAI_DICTATION_TRANSCRIPTION_MODEL", "gpt-transcribe"),
            .dictation_transcription_prompt = try envOwnedOptionalTrimmed(allocator, environ, "OPENAI_DICTATION_TRANSCRIPTION_PROMPT"),
            .dictation_transcription_keywords = try envOwnedOptionalTrimmed(allocator, environ, "OPENAI_DICTATION_TRANSCRIPTION_KEYWORDS"),
            .dictation_transcription_delay = try envOwnedOptionalTrimmed(allocator, environ, "OPENAI_DICTATION_TRANSCRIPTION_DELAY"),
            ...
            .fujin_endpoint = try envOwnedOrDefaultAny(allocator, environ, &.{ "RESONUS_FUJIN_ZMQ_ENDPOINT", "FUJIN_ZMQ_ENDPOINT" }, "tcp://127.0.0.1:5557"),
            .fujin_client_target = try envOwnedOrDefaultAny(allocator, environ, &.{ "RESONUS_FUJIN_CLIENT_TARGET", "FUJIN_CLIENT_TARGET" }, "resonus-nrpc"),
            .service_token = try envOwnedRequired(allocator, environ, "SERVICE_TOKEN"),
    ...
    pub fn deinit(self: *Config) void {
        ...
        if (self.openai_transcription_prompt) |v| a.free(v);
        if (self.openai_transcription_keywords) |v| a.free(v);
        a.free(self.dictation_transcription_model);
        if (self.dictation_transcription_prompt) |v| a.free(v);
        if (self.dictation_transcription_keywords) |v| a.free(v);
        if (self.dictation_transcription_delay) |v| a.free(v);
        ...
        a.free(self.fujin_endpoint);
        a.free(self.fujin_client_target);
...
fn envOwnedRequired(allocator: std.mem.Allocator, environ: *const std.process.Environ.Map, key: []const u8) ![]u8 {
    const value = environ.get(key) orelse return error.MissingRequiredEnv;
