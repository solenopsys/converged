...
pub fn post(
    ...
    // The client advertises gzip, and some endpoints use it (the transcriptions
    // API always does). The plain reader hands back the compressed bytes, which
    // then fail to parse as JSON — decompress according to the response header.
