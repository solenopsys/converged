...
pub fn main(init: std.process.Init) !void {
    ...
    // One process transport: one DEALER socket and identity handle incoming
    ...
    runtime.* = try transport.Runtime.init(allocator, .{
        .endpoint = fujin_endpoint,
        .target = fujin_target,
        .limits = .{ .max_envelope_bytes = 64 * 1024, .max_payload_bytes = 16 * 1024 * 1024 },
        .recv_timeout_ms = 1000,
        .send_timeout_ms = 1000,
        // The incoming handler (offer to Store) issues nested NRPC calls over
        // this transport, so it runs on a dedicated thread rather than reactor.
        .workers = 1,
    });
    ...
    if (args.len <= 1 or std.mem.eql(u8, args[1], "serve")) {
        ...
        runtime.bind("resonus", provider.transportHandler()) catch |err| {
            std.log.err("resonus transport handler registration failed: {s}", .{@errorName(err)});
            return err;
        };
        const fujin_thread = std.Thread.spawn(.{}, runtimeRun, .{runtime}) catch |err| {
            std.log.err("resonus reactor thread failed: {s}", .{@errorName(err)});
            return err;
        };
        fujin_thread.detach();
        while (!runtime.isRunning()) init.io.sleep(std.Io.Duration.fromMilliseconds(1), .awake) catch {};
        std.debug.print("[fujin] service registered target={s} endpoint={s}\n", .{ fujin_target, fujin_endpoint });
    ...
    // Non-serve subcommands still use the command layer (Store → services), so
    // the single transport's reactor must be running for `runtime.call` to
    // complete. Start it before dispatching the command.
    {
        const fujin_thread = std.Thread.spawn(.{}, runtimeRun, .{runtime}) catch |err| {
            std.log.err("resonus reactor thread failed: {s}", .{@errorName(err)});
            return err;
...
fn runtimeRun(runtime: *transport.Runtime) void {
    runtime.run();
}
