...
pub const ChatRequest = struct {
    ...
    /// Only structured orchestration steps require a tool call. Normal chat
    /// must remain free to answer without invoking a tool.
...
/// A provider writes normalized chat events to this sink as soon as it receives
/// vendor SSE data. The consumer owns sequencing and transport envelopes.
pub const StreamSink = struct {
    context: *anyopaque,
    emit_fn: *const fn (context: *anyopaque, event_json: []const u8) anyerror!void,

    pub fn emit(self: StreamSink, event_json: []const u8) !void {
        return self.emit_fn(self.context, event_json);
    }
};
...
pub const Provider = struct {
    ...
    stream: ?*const fn (ctx: *anyopaque, env: CallEnv, req: ChatRequest, sink: StreamSink) anyerror!Completion = null,
};
...
pub fn textDelta(a: std.mem.Allocator, text: []const u8) ![]const u8 {
    const encoded = try jsonStr(a, text);
    return std.fmt.allocPrint(a, "{{\"type\":\"text.delta\",\"text\":{s}}}", .{encoded});
}

pub fn toolCallDelta(a: std.mem.Allocator, id: []const u8, name: []const u8, arguments_text: []const u8) ![]const u8 {
    const id_json = try jsonStr(a, id);
    const name_json = try jsonStr(a, name);
    const args_json = try jsonStr(a, arguments_text);
    return std.fmt.allocPrint(a, "{{\"type\":\"tool_call.delta\",\"callId\":{s},\"name\":{s},\"argumentsText\":{s}}}", .{ id_json, name_json, args_json });
}

pub fn toolCallReady(a: std.mem.Allocator, call: ToolCall) ![]const u8 {
    const id_json = try jsonStr(a, call.id);
    const name_json = try jsonStr(a, call.name);
    return std.fmt.allocPrint(a, "{{\"type\":\"tool_call.ready\",\"callId\":{s},\"name\":{s},\"arguments\":{s}}}", .{
        id_json,
        name_json,
        safeJsonObject(a, call.args_json),
    });
}

/// Tool arguments arrive as incremental JSON text. Never put malformed text in
/// an outbound envelope: that would corrupt the whole stream payload.
pub fn safeJsonObject(a: std.mem.Allocator, raw: []const u8) []const u8 {
    if (raw.len == 0) return "{}";
    const value = std.json.parseFromSliceLeaky(std.json.Value, a, raw, .{}) catch return "{}";
    return if (value == .object) raw else "{}";
}

pub fn usageEvent(a: std.mem.Allocator, input: i64, output: i64) ![]const u8 {
    return std.fmt.allocPrint(a, "{{\"type\":\"usage\",\"inputTokens\":{d},\"outputTokens\":{d}}}", .{ input, output });
}

pub fn completedEvent(a: std.mem.Allocator, finish_reason: []const u8) ![]const u8 {
    const reason = try jsonStr(a, finish_reason);
    return std.fmt.allocPrint(a, "{{\"type\":\"response.completed\",\"finishReason\":{s}}}", .{reason});
}
...
pub fn boolField(v: std.json.Value, name: []const u8) ?bool {
    const f = field(v, name) orelse return null;
    return switch (f) {
        .bool => |value| value,
        else => null,
