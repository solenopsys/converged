...
pub fn make(cfg: *const Config) provider.Provider {
    return .{ .name = "claude", .ctx = @constCast(cfg), .complete = complete, .stream = stream };
}
...
fn stream(ctx: *anyopaque, env: provider.CallEnv, req: provider.ChatRequest, sink: provider.StreamSink) anyerror!provider.Completion {
    ...
    try body.appendSlice(a, "{\"model\":");
    try provider.appendJsonStr(&body, a, req.model);
    try body.appendSlice(a, try std.fmt.allocPrint(a, ",\"max_tokens\":{d},\"stream\":true", .{req.max_tokens}));
    if (req.temperature) |t|
    ...
    var emitted: usize = 0;
    for (req.messages, 0..) |m, idx| {
        const role = provider.strField(m, "role") orelse return error.ClaudeMessageInvalid;
        if (std.mem.eql(u8, role, "system")) continue;
        if (emitted > 0) try body.appendSlice(a, ",");
        emitted += 1;
        _ = idx;
    ...
    if (req.tools.len > 0) {
        ...
        for (req.tools, 0..) |t, i| {
            if (i > 0) try body.appendSlice(a, ",");
            try body.appendSlice(a, "{\"name\":");
            try provider.appendJsonStr(&body, a, provider.strField(t, "name") orelse "");
            try body.appendSlice(a, ",\"description\":");
            try provider.appendJsonStr(&body, a, provider.strField(t, "description") orelse "");
            try body.appendSlice(a, ",\"input_schema\":");
            if (provider.field(t, "parameters")) |parameters| {
                try provider.appendValue(&body, a, parameters);
            } else try body.appendSlice(a, "{\"type\":\"object\",\"properties\":{}}");
            try body.appendSlice(a, "}");
        }
        try body.appendSlice(a, "]");
    }
...
const StreamState = struct {
    ...
    fn callFor(self: *StreamState, index: i64) !*StreamCall {
        for (self.calls.items) |*call| if (call.index == index) return call;
        try self.calls.append(self.a, .{ .index = index });
...
fn onSseLine(context: *anyopaque, line: []const u8) anyerror!void {
    ...
    const root = std.json.parseFromSliceLeaky(std.json.Value, state.a, data, .{}) catch return error.ClaudeStreamJson;
    const event_type = provider.strField(root, "type") orelse return;
    if (std.mem.eql(u8, event_type, "error")) return error.ClaudeStreamFailed;
    if (std.mem.eql(u8, event_type, "message_start")) {
        if (provider.field(root, "message")) |message| {
            if (provider.field(message, "usage")) |usage| {
                state.usage_input = provider.intField(usage, "input_tokens") orelse state.usage_input;
            }
        }
        return;
    }
    if (std.mem.eql(u8, event_type, "message_delta")) {
        if (provider.field(root, "delta")) |delta| {
            if (provider.strField(delta, "stop_reason")) |reason| {
                state.finish_reason = try state.a.dupe(u8, reason);
            }
        }
        if (provider.field(root, "usage")) |usage| {
            state.usage_output = provider.intField(usage, "output_tokens") orelse state.usage_output;
        }
        return;
    }
    const index = provider.intField(root, "index") orelse return;
    if (std.mem.eql(u8, event_type, "content_block_start")) {
        const block = provider.field(root, "content_block") orelse return;
        if (!std.mem.eql(u8, provider.strField(block, "type") orelse "", "tool_use")) return;
        const call = try state.callFor(index);
        call.id = try state.a.dupe(u8, provider.strField(block, "id") orelse "");
        call.name = try state.a.dupe(u8, provider.strField(block, "name") orelse "");
        return;
    }
    if (!std.mem.eql(u8, event_type, "content_block_delta")) return;
    const delta = provider.field(root, "delta") orelse return;
    const delta_type = provider.strField(delta, "type") orelse return;
    if (std.mem.eql(u8, delta_type, "text_delta")) {
        const text = provider.strField(delta, "text") orelse return;
        try state.text.appendSlice(state.a, text);
        try state.sink.emit(try provider.textDelta(state.a, text));
    } else if (std.mem.eql(u8, delta_type, "input_json_delta")) {
        const fragment = provider.strField(delta, "partial_json") orelse return;
        const call = try state.callFor(index);
        try call.arguments.appendSlice(state.a, fragment);
        try state.sink.emit(try provider.toolCallDelta(state.a, call.id, call.name, fragment));
    }
}
