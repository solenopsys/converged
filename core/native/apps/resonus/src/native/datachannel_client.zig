...
pub const Client = struct {
    ...
    /// Send a TEXT frame. libdatachannel takes the length from the trailing NUL
    /// (negative size convention), so this is only valid for payloads that can
    /// never contain 0x00 — i.e. the JSON events on the data channel.
    ///
    /// NEVER use it for media: an Opus/RTP packet is cut at its first zero byte,
    /// which silently truncates the frame (audible as chopped/garbled audio) or,
    /// when what is left is shorter than an RTP header, fails outright with
    /// rc=-2 (RTC_ERR_FAILURE) on every single packet.
    pub fn sendText(self: *const Client, id: i32, payload: []const u8) !void {
        try self.check(self.fn_send_message(self.handle, id, payload.ptr, payload.len));
    }

    /// Send a BINARY frame with an explicit length — the only correct path for
    /// RTP/Opus media, whose bytes are arbitrary and routinely contain 0x00.
    pub fn sendBinary(self: *const Client, id: i32, payload: []const u8) !void {
