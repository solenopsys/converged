...
pub const Store = struct {
    ...
    /// The single process transport. The store's command-layer calls
    /// (service RPC) go through the same Runtime that serves inbound signaling
    /// — one socket, one identity. WebRTC/SIP are separate interfaces and are
    /// not part of this.
    ...
    pub fn init(allocator: std.mem.Allocator, runtime: *transport.Runtime, cfg: StoreConfig) !Store {
    ...
    pub fn getActiveLlmConfig(self: *Store, allocator: std.mem.Allocator, scope: []const u8) !?LlmRuntimeConfig {
        var response = try self.post("resonus", "listLlmGateConfigs", "{}", scope);
        defer response.deinit(self.allocator);
        try ensureOk(response.ok());
    ...
    pub fn putAudioFrame(self: *Store, session_id: []const u8, source: Source, timestamp_ns: i64, data: []const u8) !void {
        ...
        const cache_key = try self.redis.buildKey(self.allocator, &.{ "resonus", "fragments", session_id, source.str(), ts });
    ...
    pub fn getAudioFrames(self: *Store, allocator: std.mem.Allocator, session_id: []const u8, source: Source) ![]AudioFrame {
        const pattern = try self.redis.buildKey(self.allocator, &.{ "resonus", "fragments", session_id, source.str(), "*" });
    ...
    pub fn publishCallEvent(self: *Store, session_id: []const u8) !void {
        ...
        var resp = try self.post("events", "publish", body, "");
        defer resp.deinit(self.allocator);
        try ensureOk(resp.ok());
    }

    fn post(self: *Store, service: []const u8, method: []const u8, body: []const u8, scope: []const u8) !transport.RuntimeReply {
        return self.runtime.call(self.allocator, .{
            .service = service,
            .method = method,
            .scope = scope,
