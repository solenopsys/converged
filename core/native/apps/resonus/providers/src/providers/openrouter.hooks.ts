/**
 * Warm hooks for `openrouter` — the business logic, and nothing else.
 *
 ...
 *
 * The request body is where OpenRouter parts ways with OpenAI, and it is the
 * only place:
 *
 *   - `max_tokens`, not `max_completion_tokens`;
 *   - usage in a stream comes from a top-level `usage: {include: true}` field,
 *     not OpenAI's `stream_options`.
 *
 * Both are body-only differences, which is exactly why they belong in a hook
...
export const hooks = {
	encodeTurn(req: TurnRequest, stream: boolean): WireRequest {
		const body: Record<string, unknown> = {
			model: req.model,
			// OpenRouter reads `max_tokens`; `max_completion_tokens` is not part
			// of its schema and is silently ignored, which would drop the cap.
