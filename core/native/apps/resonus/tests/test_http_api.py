"""
HTTP API integration tests for the Resonus gateway.
...
class TestContextAPI:
    ...
    def test_set_context(self, gateway):
        r = gateway.post(f"/context/{self.PHONE}", json={"context": "keep responses brief"})
        assert r.status_code == 200
...
class TestResponseShape:
    """Verify JSON shape for key endpoints."""
