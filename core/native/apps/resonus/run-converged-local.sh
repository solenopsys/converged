...
CONTAINER_NAME="${CONTAINER_NAME:-resonus-converged}"
IMAGE="${IMAGE:-localhost/resonus:latest}"
...
FUJIN_ZMQ_ENDPOINT="${RESONUS_FUJIN_ZMQ_ENDPOINT:-${FUJIN_ZMQ_ENDPOINT:-tcp://127.0.0.1:5557}}"
FUJIN_CLIENT_TARGET="${RESONUS_FUJIN_CLIENT_TARGET:-${FUJIN_CLIENT_TARGET:-resonus-nrpc}}"
...
podman run -d --replace --name "$CONTAINER_NAME" --network host \
  ...
  -e RESONUS_FUJIN_ZMQ_ENDPOINT="$FUJIN_ZMQ_ENDPOINT" \
  -e RESONUS_FUJIN_CLIENT_TARGET="$FUJIN_CLIENT_TARGET" \
